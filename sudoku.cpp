...
complexity of this algorithm: O(n^n)
***************************************************/
#include <iostream>
...
//This function will tell if num is possible at y(row), x(column)  O(n) for this func 
bool valid(const int s[][SIZE], int y, int x, int num)
{
	    ...
	    for(int i = 1; i <= SIZE; i++) //a number can be 1 through SIZE 
	      {
		    ...
		    s[r][c] = 0;//it was a bad choice. Place 0 back in there.
		      }
          
	      }

	    //tried out num = 1 through 9, but none worked
	    return; //there is no possible answer for (r, c)
	  }//end of if(possible)
